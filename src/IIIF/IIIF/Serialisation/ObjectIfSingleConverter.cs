...
public class ObjectIfSingleConverter : JsonConverter
{
    public override void WriteJson(JsonWriter writer, object? value, JsonSerializer serializer)
    {
        ...
        if (list.Count > 1) writer.WriteStartArray();

        foreach (var element in list) serializer.Serialize(writer, element);

        if (list.Count > 1) writer.WriteEndArray();
    }
        ...
        JsonSerializer serializer)
    {
        ...
        if (reader.TokenType == JsonToken.String) return CreateListOfOne(objectType, reader.Value?.ToString());
